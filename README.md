...
Environment variables live in [.env](.env); an example is in [.env.example](.env.example). Key options:
...
Target Web URL
```bash
export ICINGA_WEB_BASE_URL="https://monitoring.example.com/icingaweb2"
```
...
Common make targets (see [Makefile](Makefile)):
...
Licensed under GPL v2 (SPDX: GPL-2.0-only). See [LICENSE](LICENSE) and https://www.gnu.org/licenses/old-licenses/gpl-2.0.txt
