...
class Settings(BaseSettings):
    ...
    username: str = Field("api-user", validation_alias=AliasChoices("ICINGA_WEB_USERNAME", "USERNAME"))
    password: SecretStr = Field(
        SecretStr("change-me"),
        validation_alias=AliasChoices("ICINGA_WEB_PASSWORD", "PASSWORD"),
    )
    # TLS
    verify_tls: bool = Field(True, validation_alias=AliasChoices("ICINGA_WEB_VERIFY_TLS", "VERIFY_TLS"))
    ca_bundle: str | None = Field(None, validation_alias=AliasChoices("ICINGA_WEB_CA_BUNDLE", "CA_BUNDLE"))

    # HTTP behavior
    request_timeout: float = Field(15.0, validation_alias=AliasChoices("HTTP_REQUEST_TIMEOUT", "REQUEST_TIMEOUT"))
    max_retries: int = Field(3, validation_alias=AliasChoices("HTTP_MAX_RETRIES", "MAX_RETRIES"))
    retry_backoff: float = Field(0.5, validation_alias=AliasChoices("HTTP_RETRY_BACKOFF", "RETRY_BACKOFF"))
    ...
    rest_auth_required: bool = Field(False, validation_alias=AliasChoices("REST_REQUIRE_API_KEY", "REQUIRE_API_KEY"))
    rest_bearer_token: SecretStr | None = Field(None, validation_alias=AliasChoices("REST_BEARER_TOKEN", "BEARER_TOKEN"))
